  ...
  return (
    <div
      className={`border rounded-xl overflow-hidden transition-all duration-200 cursor-pointer
        ${open ? "border-elevated bg-raised" : "border-border bg-base hover:border-elevated"}`}
      onClick={() => setOpen((p) => !p)}>
      <div className="flex items-center justify-between px-5 py-4">
        ...
        <ChevronDown
          size={16}
          className={`text-muted-foreground transition-transform duration-300 shrink-0 ml-4 ${open ? "rotate-180" : ""}`}
        />
      </div>
      <div
